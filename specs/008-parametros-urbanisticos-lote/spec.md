...
- FR-009: Un 5xx de la capa SDP DEBE degradar el bloque `urbanistic_parameters` a estado `no_encontrado` con warning `BLOQUE_DEGRADADO` (alineado con FR-008): nunca es fatal para el informe y NUNCA se reporta como error `FUENTE_5XX`, pues se trata de un bloque auxiliar con degradación por bloque (estilo F6/F7). La causa 5xx queda registrada en el mensaje del warning.
- FR-010: El bloque DEBE incluir `source_trace` con los 5 campos: `source_name`, `layer_id`, `service_url`, `data_vigencia`, `query_timestamp`. Siguiendo el patrón de F6/F7 (un solo `source_trace` por bloque), el trace documenta la fuente primaria del tratamiento espacial (SINUPOT/SDP); la proveniencia de los parámetros numéricos del RAG NO genera un trace adicional y queda registrada en `interpretation` y en los warnings del informe.
