...
- **URL de origen no disponible durante la descarga**: error tipificado de fuente — en la ingesta CLI, fallo atómico por documento con error accionable (`RuntimeError` de `cmd_descargar`, exit code **3** vía el handler genérico de `main()` de la ingesta, corpus.py:1048); en las tools MCP, `FUENTE_5XX` — sin corromper el corpus.
