...
- `speckit.plan` y `speckit.tasks` leen `.specify/memory/constitution.md` (ratificada
  2026-08-10, v1.0.1 — enmienda PATCH F12: stdio por defecto + HTTP opcional,
  aprobada por el usuario 2026-09-20).
...
- App objetivo: Python (`mcp>=1.0.0` que incluye FastMCP, `httpx`, `pydantic`); MCP por
  stdio (default) y modo Streamable HTTP opcional (`--transport http`, F12);
...
- Herramientas MCP: **7 implementadas** (`resolve_lot_by_chip`, `resolve_lot_by_address`,
  ...
  a MCPServer (mcp 2.x); transporte stdio por defecto — Streamable HTTP opcional con
  validación `Origin`/`Host` (TransportSecuritySettings del SDK) y bind loopback (F12);
